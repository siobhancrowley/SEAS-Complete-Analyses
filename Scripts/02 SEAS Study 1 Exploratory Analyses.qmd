...
VG, SG, EX, FR, OR, age x VG, and age x SG are significant
...
ggplot(seas_separated, aes(x = age_months, y = EX_total)) + 
  geom_jitter() +
  geom_smooth(method = "lm") #probably move this/reorganize at some point
```
...
summary(lm(
  ...
  data = seas_EX_qtype
)) #probably move again; not significant; double-check lm, lmer, glmer
```
...
Age, age x PR, age x MT significant. Double-check this because this doesn't seem to match the visual at all?
