...
summary(seas_EX_count_age)$coefficients[2,4] * 2 #Bonferroni corrected p
```

The number of explanations that reference exclusion given by participants intitially appears to significantly differ by age, t = `r round(summary(seas_EX_count_age)$coefficients[2,3], 2)`, p = `r signif(summary(seas_EX_count_age)$coefficients[2,4], 2)`, such that older participants are more likely to use a higher frequency of EX explanations, but is not significant when a Bonferroni correction for multiple comparisons is applied (p = `r signif(summary(seas_EX_count_age)$coefficients[2,4] * 2, 2)`).
...
summary(seas_SG_count_age)$coefficients[2,4] * 2 #Bonferroni corrected p

confint(seas_SG_count_age, method = "Wald", parm = "age_months") #fixed effect confidence interval

exp(coef(seas_SG_count_age)) #odds ratio
```

The number of explanations that reference the game with specific details given by participants does significantly differ by age, t = `r round(summary(seas_SG_count_age)$coefficients[2,3], 2)`, p = `r signif(summary(seas_SG_count_age)$coefficients[2,4], 2)`, and remains significant after correction (p = `r signif(summary(seas_SG_count_age)$coefficients[2,4] * 2, 2)`).
...
seas_status_degrees <- seas_count_status_cor_matrix$n[1, 1] #Confirmed ns the same within status
...
seas_resource_degrees <- seas_count_resource_cor_matrix$n[1, 1]
...
#Does the presence of explanations that reference authority change with age?
...
seas_AR_present_age <- summary(glm(
...
Whether participants reference authority at least once in the study does not significantly differ with age, t = `r round(seas_AR_present_age$coefficients[2,3], 2)`, p = `r signif(seas_AR_present_age$coefficients[2,4], 2)`.

```{r AR-present-age-scatter}
AR_present_age_scatter <- seas_separated |>
  ...
  labs(
    title = "Study 1 Explanations Referencing Authority by Age",
    x = "Age (Months)",
    y = NULL
  ) +
  theme_minimal() +
...
summary(seas_EX_present_age)$coefficients[2,4] * 7 #Bonferroni-corrected p

confint(seas_EX_present_age, method = "Wald", parm = "age_months") #fixed effect confidence interval

exp(coef(seas_EX_present_age)) #odds ratio
```

Whether participants reference exclusion at least once in the study significantly differs with age, t = `r round(summary(seas_EX_present_age)$coefficients[2,3], 2)`, p = `r signif(summary(seas_EX_present_age)$coefficients[2,4], 2)`, and this survives a Bonferroni correction for multiple comparisons (p = `r signif(summary(seas_EX_present_age)$coefficients[2,4] * 7, 2)`)
...
summary(seas_FR_present_age)$coefficients[2,4] * 7 #Bonferroni-corrected p
```

Whether participants reference friends at least once in the study significantly differs with age, t = `r round(summary(seas_FR_present_age)$coefficients[2,3], 2)`, p = `r signif(summary(seas_FR_present_age)$coefficients[2,4], 2)`, but this does not survive correction (p = `r signif(summary(seas_FR_present_age)$coefficients[2,4] * 7, 2)`). 
...
summary(seas_PS_present_age)$coefficients[2,4] * 7 #Bonferroni-corrected p
```

Whether participants reference possessions at least once in the study appears to significantly differ with age, t = `r round(summary(seas_PS_present_age)$coefficients[2,3], 2)`, p = `r signif(summary(seas_PS_present_age)$coefficients[2,4], 2)`, but does not survive correction (p = `r signif(summary(seas_PS_present_age)$coefficients[2,4] * 7, 2)`).
...
summary(seas_SG_present_age)$coefficients[2,4] * 7 #Bonferroni corrected p

confint(seas_SG_present_age, method = "Wald", parm = "age_months") #fixed effect confidence interval

exp(coef(seas_SG_present_age)) #odds ratio
```

Whether participants reference the game specifically at least once in the study significantly differs with age, t = `r round(summary(seas_SG_present_age)$coefficients[2,3], 2)`, p = `r signif(summary(seas_SG_present_age)$coefficients[2,4], 2)`, and survives correction (p = `r signif(summary(seas_SG_present_age)$coefficients[2,4] * 7, 2)`). 
...
Whether participants reference superficial traits at least once in the study does not significantly differ with age, t = `r round(summary(seas_ST_present_age)$coefficients[2,3], 2)`, p = `r signif(summary(seas_ST_present_age)$coefficients[2,4], 2)`. 
...
summary(seas_VG_present_age)$coefficients[2,4] * 7 #Bonferroni-corrected p
```

Whether participants reference the game vaguely at least once in the study appeared to significantly differ with age, t = `r round(summary(seas_VG_present_age)$coefficients[2,3], 2)`, p = `r signif(summary(seas_VG_present_age)$coefficients[2,4], 2)`, but did not survive correction (p = `r signif(summary(seas_VG_present_age)$coefficients[2,4] * 7, 2)`). 
...
confint(seas_word_glm, method = "Wald", parm = "age_months") #fixed effect confidence interval

exp(coef(seas_word_glm)) #odds ratio
```

The number of words used by participants significantly differs based on age, t = `r round(summary(seas_word_glm)$coefficients[2,3], 2)`, p = `r signif(summary(seas_word_glm)$coefficients[2,4], 2)`.
